class Api::V1::WeathersController < ApplicationController
  ...
  def fetch_existing_dates_data(city_id, base_date)
    ...
    dates_to_check.map do |date|
      Weather.find_by(city_id: city_id, date_time: date.strftime('%Y-%m-%d %H:%M:%S'))
    end.compact
  end
  ...
  def fetch_existing_days_data(city_id, base_date)
    ...
    days_to_check.map do |day|
      Weather.find_by(city_id: city_id, date_time: day.strftime('%Y-%m-%d %H:%M:%S'))
    end.compact
  end
